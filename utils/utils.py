...
def import_data(file_dir) -> pd.DataFrame:
    """
    ...
    try:
        ...
        logging.info(f'Data has been imported')
        return df
...
def create_dir(path: Path):
    try:
        if path.is_dir() == False:
            os.makedirs(path)
            logging.info(f' Directory created at the '
                         f'requested path {path}')

    except SyntaxError as e:
        logging.error(f'During folder creation {e}')
...
def save_model(path, model):
    try:
        ...
        logging.error(f'model not dumped {Exception}')
...
# Taken from https://www.kaggle.com/code/drazen/heatmap-with-sized-markers/notebook
